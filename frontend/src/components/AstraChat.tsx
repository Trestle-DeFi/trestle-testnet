  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 w-80 sm:w-96 bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden flex flex-col" style={{ maxHeight: "70vh" }}>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2 min-h-[200px]">
        ...
        {busy && <div className="text-xs text-gray-400 animate-pulse pl-1">Astra is thinking...</div>}
        <div ref={bottom} />
      </div>
      <div className="border-t p-3 flex gap-2">
        <input value={input} onChange={e => setInput(e.target.value)}
          onKeyDown={e => e.key === "Enter" && send()}
          placeholder="Ask Astra..." className="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-emerald-400" />
